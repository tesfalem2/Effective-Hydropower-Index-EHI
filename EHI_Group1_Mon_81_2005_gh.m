...
function [EHI_mon,EHI_dry,EHI_wet,EHI_longterm,EHI_yearly] = EHI_Group1_Mon_81_2005_gh(q_in,q_turb)
% Inputs
...
startYear = 1981; % always only change this data
endYear = 2005;

inial_YR = startYear-1; % 
YearNum = [startYear:endYear]; % simulation periods
...
% pre-allocation for Vinflow and Vturb on yearly and monthly
Vin_yr = zeros(nYear,1);
Vturb_yr = zeros(nYear,1);

% Annual Volume and Annual EHI calculation
for j=startYear:endYear  % for year 1981:2005
    Yearly_Indx = V(:,1) == j;
